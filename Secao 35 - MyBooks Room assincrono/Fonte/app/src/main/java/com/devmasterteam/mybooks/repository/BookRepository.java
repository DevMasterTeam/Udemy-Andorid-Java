...
public class BookRepository {
    ...
    // Executor para rodar as operações do banco de dados em uma thread separada
    ...
    public void getAllBooks(Callback<List<BookEntity>> callback) {
        executor.execute(new Runnable() {
            @Override
            public void run() {
                callback.onSuccess(dataBase.getAllBooks());
            }
        });
    }
     ...
     */
    public void deleteBook(int id, Callback<Boolean> callback) {
        executor.execute(new Runnable() {
            @Override
            public void run() {
                BookEntity book = dataBase.getBookById(id);
